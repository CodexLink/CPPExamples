/* Laboratory Activity 6, Selection Sort on Strings, By Janrey Licas, CPE21FB1
...
#define SORT_TYPE "Selection Sort"
...
int main()
{
    ...
    {
        unsigned short MinElem = FirstElem;
        for (auto SecondElem = MinElem + POS_OFFSET_BY_ONE; SecondElem < MAX_CONSTRAINT; SecondElem++)
        {
            (LN_Container[SecondElem].compare(LN_Container[MinElem]) < STR_EQUAL) ? MinElem = SecondElem : DO_NOTHING;
        }
        swap(LN_Container[MinElem], LN_Container[FirstElem]);
