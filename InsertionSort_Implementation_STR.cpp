/* Laboratory Activity 6, Insertion Sort on Strings, By Janrey Licas, CPE21FB1
...
#define SORT_TYPE "Insertion Sort"
...
int main()
{
    ...
    for (auto ArrElem = POS_OFFSET_BY_ONE; ArrElem < MAX_CONSTRAINT; ArrElem++)
    {
        std::string ArrElemChosen = LN_Container[ArrElem];
        unsigned int ArrReadBack_Index = ArrElem - POS_OFFSET_BY_ONE;

        while (ArrReadBack_Index >= 0 && (LN_Container[ArrReadBack_Index].compare(ArrElemChosen) > STR_EQUAL))
        {
            LN_Container[ArrReadBack_Index + POS_OFFSET_BY_ONE].assign(LN_Container[ArrReadBack_Index]), ArrReadBack_Index--;
        }
        LN_Container[ArrReadBack_Index + POS_OFFSET_BY_ONE].assign(ArrElemChosen);
...
void DisplayArray_SortProcess(std::string ArrayContainer[MAX_CONSTRAINT])
{
    ...
    {
        std::cout << ArrayContainer[DisplayArray] << " ";
    }
